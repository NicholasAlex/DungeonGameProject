...
//==================================================================
{
	...
	for (int i = 0; i < gItems.size(); i++)
	{
		...
		if (gItems[i]->getIcon() != 'T' && gItems[i]->update(gHero) && gItems[i]->getAppearance() <= gameLevel + 1)
		{
		...
		// If it is trigger, update it and increment the appearance
		else if (gItems[i]->getIcon() == 'T' && gItems[i]->update(gHero) && gItems[i]->getAppearance() <= gameLevel + 1)
		{
			gItems[i]->setAppearance(gItems[i]->getAppearance() + 1);
		}
		// If the item appearance in the current level is bigger than the game level + 1, remove the item
		else if (gItems[i]->getAppearance() > gameLevel + 1)
		{
			gItems[i]->remove();
		}
	}
